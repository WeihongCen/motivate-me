            ...
            options: {
                ...
                onClick: async (e, element) => {
                    if (element.length) {
                        ...
                        const analysisTimeKey = Math.floor(selectedTime / 300000) * 300000;
                        selectedAnalysis = Highlight.appStorage.get(`analysis/${analysisTimeKey}`);
                        appDetails = {};
                        for (let i = analysisTimeKey; i < analysisTimeKey + 300000; i += 10000) {
...
<div class="flex flex-col gap-5">
    ...
    {#if selectedAnalysis}
        <p class={`${selectedAnalysis.productive ? `text-[#86efac]` : `text-[#fca5a5]`}`}>
            {selectedAnalysis.productive ? "productive" : "unproductive"}
        </p>
